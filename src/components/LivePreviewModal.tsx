...
// Generate the inner content HTML for an addon section (without the wrapper div)
function getAddonInnerHtml(addon: Addon): string {
  ...
  if (lower.includes('music') || lower.includes('musica')) {
    ...
    return `
        ...
        <div style="max-width:350px;margin:0 auto;">
          ...
          <select style="width:100%;padding:12px 16px;border:1px solid rgba(255,255,255,0.2);background:rgba(255,255,255,0.1);border-radius:10px;color:white;font-size:14px;margin-bottom:12px;box-sizing:border-box;">
            <option>Parteciper&ograve; con gioia!</option><option>Purtroppo non posso</option>
          </select>
    ...
    return `
        ...
        <div style="max-width:400px;margin:0 auto;display:flex;flex-direction:column;gap:8px;">
          <div style="background:white;border-radius:12px;padding:14px 16px;text-align:left;box-shadow:0 2px 8px rgba(0,0,0,0.04);">
            <div style="font-size:12px;font-weight:600;color:#333;">Mamma e Pap&agrave;</div>
            <div style="font-size:11px;color:#888;margin-top:4px;font-style:italic;">"Siamo cos&igrave; felici per voi! Vi auguriamo tutto l'amore del mondo."</div>
          </div>
          <div style="background:white;border-radius:12px;padding:14px 16px;text-align:left;box-shadow:0 2px 8px rgba(0,0,0,0.04);">
    ...
    return `
        ...
        <div style="max-width:260px;margin:0 auto;background:white;border-radius:20px;padding:28px;box-shadow:0 8px 32px rgba(0,0,0,0.08);">
          ...
          <div style="font-size:10px;color:#aaa;margin-top:4px;">15 Giugno 2026 &bull; Roma</div>
    ...
    return `
        ...
        <div style="font-size:22px;font-weight:300;color:#333;margin-bottom:16px;">Come Tutto &Egrave; Iniziato</div>
        <div style="max-width:450px;margin:0 auto;font-size:13px;color:#666;line-height:2;">
          Ci siamo incontrati in una sera d'estate. Un sorriso ha cambiato tutto. Da quel momento, ogni giorno &egrave; stato un'avventura meravigliosa che ci ha portato fin qui.
        </div>`;
    ...
    return `
        ...
        <div style="max-width:350px;margin:0 auto;background:white;border-radius:16px;padding:20px;box-shadow:0 2px 12px rgba(0,0,0,0.06);">
          <div style="font-size:13px;color:#555;line-height:1.8;">La vostra presenza &egrave; il regalo pi&ugrave; bello. Se desiderate farci un pensiero:</div>
    ...
    return `
        ...
        <div style="max-width:450px;margin:0 auto;text-align:left;">
          <div style="background:white;border-radius:12px;padding:14px 16px;margin-bottom:8px;box-shadow:0 1px 4px rgba(0,0,0,0.04);">
            ...
            <div style="font-size:12px;color:#888;margin-top:4px;">S&igrave;, l'invito &egrave; esteso al tuo +1.</div>
          </div>
          <div style="background:white;border-radius:12px;padding:14px 16px;box-shadow:0 1px 4px rgba(0,0,0,0.04);">
            <div style="font-size:13px;font-weight:600;color:#333;">C'&egrave; parcheggio?</div>
            <div style="font-size:12px;color:#888;margin-top:4px;">S&igrave;, parcheggio gratuito presso la location.</div>
          </div>
        </div>`;
...
// Get the wrapper style for an addon section
function getAddonWrapperStyle(addon: Addon): string {
  const lower = addon.name.toLowerCase();
  if (lower.includes('music') || lower.includes('musica')) return 'padding:48px 24px;background:linear-gradient(135deg,#1a1a2e,#16213e);text-align:center;position:relative;overflow:hidden;min-height:200px;';
  if (lower.includes('galleria') || lower.includes('gallery') || lower.includes('foto')) return 'padding:48px 24px;background:#fafafa;text-align:center;position:relative;overflow:hidden;min-height:280px;';
  if (lower.includes('countdown') || lower.includes('timer')) return 'padding:48px 24px;background:linear-gradient(135deg,#f8f5ff,#fff);text-align:center;position:relative;overflow:hidden;min-height:200px;';
  if (lower.includes('rsvp') || lower.includes('conferma')) return 'padding:48px 24px;background:linear-gradient(135deg,#6366f1,#8b5cf6);text-align:center;position:relative;overflow:hidden;min-height:280px;';
  if (lower.includes('guestbook') || lower.includes('libro') || lower.includes('messaggi vocali')) return 'padding:48px 24px;background:linear-gradient(135deg,#fdf2f8,#fff);text-align:center;position:relative;overflow:hidden;min-height:240px;';
  if (lower.includes('dominio') || lower.includes('domain')) return 'padding:16px 24px;background:linear-gradient(90deg,#6366f1,#8b5cf6);text-align:center;position:relative;overflow:hidden;';
  if (lower.includes('invito') || lower.includes('invit')) return 'padding:48px 24px;background:linear-gradient(135deg,#f5f3ff,#fff);text-align:center;position:relative;overflow:hidden;min-height:260px;';
  if (lower.includes('mappa') || lower.includes('map')) return 'padding:48px 24px;background:#f8f9fa;text-align:center;position:relative;overflow:hidden;min-height:240px;';
  if (lower.includes('timeline') || lower.includes('programma')) return 'padding:48px 24px;background:white;text-align:center;position:relative;overflow:hidden;min-height:240px;';
  if (lower.includes('story') || lower.includes('storia')) return 'padding:48px 24px;background:linear-gradient(135deg,#fff5f5,#fff);text-align:center;position:relative;overflow:hidden;min-height:220px;';
  if (lower.includes('lista nozze') || lower.includes('iban') || lower.includes('regalo')) return 'padding:48px 24px;background:linear-gradient(135deg,#fef3c7,#fff);text-align:center;position:relative;overflow:hidden;min-height:200px;';
  if (lower.includes('save the date')) return 'padding:48px 24px;background:linear-gradient(135deg,#fce7f3,#fff);text-align:center;position:relative;overflow:hidden;min-height:200px;';
  if (lower.includes('faq') || lower.includes('domand')) return 'padding:48px 24px;background:#f9fafb;text-align:center;position:relative;overflow:hidden;min-height:200px;';
  if (lower.includes('dress code') || lower.includes('abbigliamento')) return 'padding:48px 24px;background:linear-gradient(135deg,#faf5ff,#fff);text-align:center;position:relative;overflow:hidden;min-height:180px;';
  if (lower.includes('video') || lower.includes('welcome')) return 'padding:48px 24px;background:linear-gradient(135deg,#111827,#1f2937);text-align:center;position:relative;overflow:hidden;min-height:260px;';
  if (lower.includes('album') || lower.includes('post-wedding')) return 'padding:48px 24px;background:linear-gradient(135deg,#fffbeb,#fff);text-align:center;position:relative;overflow:hidden;min-height:220px;';
  return 'padding:36px 24px;background:#f8f9fa;text-align:center;position:relative;overflow:hidden;min-height:120px;';
}

// Build the full addon section HTML with overlay/badge
function getAddonSectionHtml(addon: Addon, isLocked: boolean): string {
  ...
  const wrapperStyle = getAddonWrapperStyle(addon);
  const innerHtml = getAddonInnerHtml(addon);

  return `
      <div id="addon-${addon.id}" class="addon-section" data-addon-id="${addon.id}" style="${wrapperStyle}">
...
function buildEnhancedHtml(
  ...
  const visibleAddons = allAddons.filter(a => {
  ...
  const addonSections = visibleAddons
    .map(addon => {
  ...
  const animations = `
    <style>
      ...
      @keyframes addonHighlight { 0% { box-shadow: 0 0 0 0 rgba(99,102,241,0.6); } 100% { box-shadow: 0 0 0 0 transparent; } }
      .addon-highlight { animation: addonHighlight 1s ease-out; }
    </style>
  `;

  // Script that handles in-place DOM updates from parent (no iframe reload needed)
  const iframeScript = `
    <script>
      document.addEventListener('DOMContentLoaded', function() {
        ...
        // Listen for update messages from parent to update addon overlays in-place
        window.addEventListener('message', function(e) {
          if (e.data && e.data.type === 'updateAddon') {
            var addonId = e.data.addonId;
            var isSelected = e.data.isSelected;
            var addonName = e.data.addonName;
            var addonPrice = e.data.addonPrice;
            var section = document.getElementById('addon-' + addonId);
            if (!section) return;

            // Remove existing overlay and badge
            var existingOverlay = section.querySelector('.addon-overlay');
            var existingBadge = section.querySelector('.addon-selected-badge');
            ...
            if (isSelected) {
              ...
              badge.className = 'addon-selected-badge';
              badge.setAttribute('data-addon-id', addonId);
              badge.style.cssText = 'position:absolute;top:12px;right:12px;background:linear-gradient(135deg,#22c55e,#16a34a);color:white;padding:5px 14px;border-radius:20px;font-size:11px;font-weight:700;z-index:10;display:flex;align-items:center;gap:5px;box-shadow:0 2px 8px rgba(34,197,94,0.3);cursor:pointer;';
              badge.innerHTML = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg> Incluso &bull; clicca per rimuovere';
              badge.onclick = function() { window.parent.postMessage({type:'toggleAddon',addonId:addonId},'*'); };
              section.insertBefore(badge, section.firstChild);
            } else {
              // Add lock overlay
              var overlay = document.createElement('div');
              overlay.className = 'addon-overlay';
              overlay.setAttribute('data-addon-id', addonId);
              overlay.style.cssText = 'position:absolute;inset:0;background:linear-gradient(135deg,rgba(255,255,255,0.88),rgba(245,240,255,0.92));backdrop-filter:blur(8px);z-index:10;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;cursor:pointer;transition:all 0.3s ease;';
              overlay.innerHTML = '<div style="width:48px;height:48px;border-radius:50%;background:linear-gradient(135deg,#6366f1,#8b5cf6);display:flex;align-items:center;justify-content:center;box-shadow:0 4px 20px rgba(99,102,241,0.3);"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg></div><div style="font-size:15px;font-weight:700;color:#1a1a2e;text-align:center;margin-top:4px;">' + addonName + '</div><div style="font-size:20px;font-weight:800;color:#6366f1;">+' + addonPrice + '</div><div style="margin-top:4px;background:linear-gradient(135deg,#6366f1,#8b5cf6);color:white;padding:10px 32px;border-radius:50px;font-size:13px;font-weight:700;letter-spacing:0.03em;box-shadow:0 4px 16px rgba(99,102,241,0.3);display:flex;align-items:center;gap:6px;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg> Aggiungi al sito</div><div style="font-size:10px;color:#888;margin-top:2px;">Sezione extra a pagamento</div>';
              overlay.onclick = function() { window.parent.postMessage({type:'toggleAddon',addonId:addonId},'*'); };
              section.insertBefore(overlay, section.firstChild);
            }

            // Scroll to the section and highlight it
            section.scrollIntoView({ behavior: 'smooth', block: 'center' });
            section.classList.remove('addon-highlight');
            void section.offsetWidth; // force reflow
            section.classList.add('addon-highlight');
          }

          // Handle scrollToAddon message
          if (e.data && e.data.type === 'scrollToAddon') {
            var el = document.getElementById('addon-' + e.data.addonId);
            if (el) {
              el.scrollIntoView({ behavior: 'smooth', block: 'center' });
              el.classList.remove('addon-highlight');
              void el.offsetWidth;
              el.classList.add('addon-highlight');
            }
          }
        });
      });
  ...
  result = result.replace('</body>', `${animations}${iframeScript}</body>`);
...
export default function LivePreviewModal({
  ...
  // Track initial HTML to load only once; updates happen via postMessage
  const [initialHtml, setInitialHtml] = useState('');
  const prevSelectedRef = useRef<string[]>([]);

  // Build initial HTML only once when modal opens
  useEffect(() => {
    if (isOpen && !initialHtml) {
      setInitialHtml(buildEnhancedHtml(htmlContent, allAddons, selectedAddonIds));
      prevSelectedRef.current = [...selectedAddonIds];
    }
    if (!isOpen) {
      setInitialHtml('');
      prevSelectedRef.current = [];
    }
  }, [isOpen, htmlContent, allAddons, selectedAddonIds, initialHtml]);

  // When selectedAddonIds change, send postMessage to iframe to update in-place (no reload)
  useEffect(() => {
    if (!isOpen || !initialHtml || !iframeRef.current) return;
    const prev = prevSelectedRef.current;
    const curr = selectedAddonIds;
    ...
    changedIds.forEach(addonId => {
      const addon = allAddons.find(a => a.id === addonId);
      if (!addon) return;
      const isSelected = curr.includes(addonId);
      try {
        iframeRef.current?.contentWindow?.postMessage({
          ...
          addonPrice: formatPrice(addon.price),
        }, '*');
      } catch {
  ...
  }, [selectedAddonIds, isOpen, initialHtml, allAddons]);
  ...
  // Handle sidebar addon click: toggle + scroll iframe to that section
  const handleSidebarToggle = useCallback((addon: Addon) => {
    onToggleAddon(addon);
    // Scroll the iframe to show the addon section
    setTimeout(() => {
      try {
        iframeRef.current?.contentWindow?.postMessage({
          type: 'scrollToAddon',
            ...
            <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1.5">
              {filteredAddons.map((addon) => {
                ...
                return (
                  <button
                    key={addon.id}
                    onClick={() => handleSidebarToggle(addon)}
